...
export default function Navbar({ onWishlistToggle }: NavbarProps) {
    ...
    useEffect(() => {
        ...
        const wishlistUnsub = wishlistService.subscribe(() => {
            setWishlistCount(wishlistService.getWishlistCount());
        });
        ...
        <nav className="fixed top-0 left-0 right-0 z-50 bg-gray-900 text-white px-6 py-4 flex justify-between items-center shadow">
            ...
            <div className="flex items-center gap-6">
                ...
                >
                    ...
                    />
                    {wishlistCount > 0 && (
                        <span className="absolute -top-2 -right-2 h-5 w-5 text-xs font-medium bg-red-500 text-white rounded-full flex items-center justify-center animate-in zoom-in-50">
                            {wishlistCount}
                        </span>
                    )}
                </button>
